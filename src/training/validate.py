""" Validation script for Fetal Head Biometry HQNN. """
import torch
import numpy as np
...
def validate(model, dataloader, criterion, device):
    ...
    # Evaluate BPD (cols 0-3) and OFD (cols 4-7)
    mae_bpd = np.mean(np.abs(preds_arr[:, :4] - targets_arr[:, :4]), axis=1)
    mae_ofd = np.mean(np.abs(preds_arr[:, 4:] - targets_arr[:, 4:]), axis=1)
