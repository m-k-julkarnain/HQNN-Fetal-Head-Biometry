""" Master LODO Ablation Pipeline for Fetal Head Biometry with SWA. """
import os
import random
import argparse
import numpy as np
...
def train_and_evaluate(model, model_name, run_name, train_loader, val_loader, device, hf_api, repo_id):
    ...
    # 3. Stochastic Weight Averaging (SWA) Setup
    swa_model = AveragedModel(model)
    swa_start = int(EPOCHS * 0.75) # Trigger SWA at Epoch 45
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < (best_val_loss - MIN_DELTA):
            ...
            patience_counter = 0
            # Unwrap SWA module to save clean state dict
    ...
    # Generate Publication Grid using the best validation model
    # Reload best weights to ensure the grid is perfect
